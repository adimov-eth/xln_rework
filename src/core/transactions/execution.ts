import { type Hash, type Transaction } from "../../types/core";
import type { BaseMachine } from "../machines/base";
import type { ChannelMachine } from "../machines/channel";
import type { EntityMachine } from "../machines/entity";
import type { ServerMachine } from "../machines/server";
import type { SignerMachine } from "../machines/signer";
import type { Mempool } from "./mempool";
import { type TransactionValidator } from "./validation";
	...
	async executeTransaction(
		...
		try {
			...
			switch (machine.getType()) {
				case "server":
					result = await this.executeServerTransaction(
						transaction,
						machine as ServerMachine,
						executor,
					);
					break;
				case "signer":
					result = await this.executeSignerTransaction(
						transaction,
						machine as SignerMachine,
						executor,
					);
					break;
				case "entity":
					result = await this.executeEntityTransaction(
						transaction,
						machine as EntityMachine,
						executor,
					);
					break;
				case "channel":
					result = await this.executeChannelTransaction(
						transaction,
						machine as ChannelMachine,
						executor,
					);
					break;
				default:
	...
	private async executeServerTransaction(
		...
		try {
			...
			return {
				...
				gasUsed,
				error:
					error instanceof Error ? error.message : "Server execution failed",
			};
		}
	}
	...
	private async executeSignerTransaction(
		...
		try {
			...
			return {
				...
				gasUsed,
				error:
					error instanceof Error ? error.message : "Signer execution failed",
			};
		}
	}
	...
	private async executeEntityTransaction(
		...
		try {
			...
			return {
				...
				gasUsed,
				error:
					error instanceof Error ? error.message : "Entity execution failed",
			};
		}
	}
	...
	private async executeChannelTransaction(
		...
		try {
			...
			return {
				...
				gasUsed,
				error:
					error instanceof Error ? error.message : "Channel execution failed",
			};
		}
	}
