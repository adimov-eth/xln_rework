import { MessageType, type Transaction } from "../../types/core";
...
export class Mempool {
	...
	removeTransaction(transaction: Transaction): boolean {
		const index = this.priorityQueue.findIndex(
			(tx) =>
				tx.from.equals(transaction.from) && tx.nonce === transaction.nonce,
		);
	...
	getPoolSize(machineId: string): number {
		...
		return (
			pool.signerTransactions.length +
	...
	hasNonceConflict(transaction: Transaction): boolean {
		return this.priorityQueue.some(
			(tx) =>
				tx.from.equals(transaction.from) && tx.nonce === transaction.nonce,
		);
	}
	...
	getStatistics(): {
		...
		return {
			...
			averagePoolSize:
				this.pools.size > 0 ? this.priorityQueue.length / this.pools.size : 0,
			typeDistribution,
		};
	}
}
