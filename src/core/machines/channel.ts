...
import { MachineType, Message, Transaction, Hash, ChannelState, ChannelStatus } from '../../types/core';
...
export class ChannelMachine extends BaseMachine {
    ...
    constructor(id: Buffer, parent: BaseMachine, peerEntityId: Buffer, initialBalance: bigint) {
        ...
        this.channelState = {
            ...
            balances: new Map([
                ['ETH', { left: initialBalance, right: 0n }]
            ]),
            nonce: 0,
    ...
    async makePayment(asset: string, amount: bigint, recipient: Buffer): Promise<boolean> {
        const balance = this.channelState.balances.get(asset);
        if (!balance) {
            return false;
        }

        const isLeftToRight = this.channelState.participants[0].equals(this.parent!.getId());
        const senderBalance = isLeftToRight ? balance.left : balance.right;
        const senderCredit = isLeftToRight ? this.channelState.creditLimits.left : this.channelState.creditLimits.right;
    ...
    getBalance(asset: string, participant?: Buffer): { left: bigint; right: bigint } | bigint {
        const balance = this.channelState.balances.get(asset);
        if (!balance) {
    ...
    getAvailableCapacity(asset: string, participant: Buffer): bigint {
        ...
        const isLeft = this.channelState.participants[0].equals(participant);
        const creditLimit = isLeft ? this.channelState.creditLimits.left : this.channelState.creditLimits.right;
