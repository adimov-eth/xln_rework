...
import { MachineType, Message, Transaction, Hash } from '../../types/core';
...
export class SignerMachine extends BaseMachine {
    ...
    async createEntity(entityId: Buffer, isMultiSig: boolean = false): Promise<EntityMachine> {
    ...
    async processBlock(): Promise<Hash> {
        ...
        const signerRoot = entityRoots.length > 0
