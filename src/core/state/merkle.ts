import { TreeConfig } from '../../types/core';
import { hashChildren, hashValues, keccak256Hash } from '../../crypto/hashing';
import { pathToNibbles, formatPath, parsePath } from '../../utils/encoding';
...
export class MerkleTree {
    ...
    private setAtPath(node: MerkleNode, remainingPath: string, fullPath: string, value: Buffer): void {
        if (node.type === 'leaf') {
            ...
            const chunk = parseInt(remainingPath[0], 16);
    ...
    private visualizeNode(node: MerkleNode, prefix: string, isLast: boolean): string {
        ...
        if (node.type === 'leaf') {
            ...
            const hashStr = node.hash ? node.hash.toString('hex').substring(0, 8) + '...' : 'dirty';
            ...
            const sortedChildren = Array.from(node.children.entries()).sort(([a], [b]) => a - b);
