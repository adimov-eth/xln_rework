...
export class XLNRestServer extends EventEmitter {
    ...
    private setupRoutes(): void {
        ...
        apiV1.post('/entities/:id/proposals/:pid/vote', this.handleVoteProposal.bind(this));
    ...
    private async handleCreateSigner(req: Request, res: Response): Promise<void> {
        try {
            ...
            const result = await this.emitAPICall('signer.create', { signerId, privateKey });
            this.sendResponse(res, result);
        } catch (error) {
    ...
    private async handleGetEntityState(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
            const result = await this.emitAPICall('entity.getState', { entityId: id });
            this.sendResponse(res, result);
        } catch (error) {
    ...
    private async handleUpdateChannel(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
    ...
    private async handleCloseChannel(req: Request, res: Response): Promise<void> {
        try {
            ...
            const result = await this.emitAPICall('channel.close', { channelId: id, cooperative });
            this.sendResponse(res, result);
        } catch (error) {
    ...
    private sendError(res: Response, code: number, message: string, details?: any): void {
