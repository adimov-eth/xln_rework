...
export function nibblesToPath(nibbles: string, bitWidth: number = 4): Buffer {
    const bytes: number[] = [];
    let currentByte = 0;
    let bitsInByte = 0;
...
export function parsePath(formattedPath: string): { nibbles: string; count: number } {
    const count = parseInt(formattedPath.substring(0, 2), 16);
